...
const monthTransactions = useMemo(() => {
  ...
  const DateGroup = ({ date, items }) => {
    ...
    return (
      <Card noPadding className="mb-4">
        <div className="flex justify-between items-center bg-surface-50 px-4 py-2 border-b border-soft">
          <div className="flex items-end gap-3">
            <span className="text-2xl font-extrabold text-main leading-none">{day}</span>
            <div className="flex flex-col gap-0 leading-tight">
              <Badge>{dayName}</Badge>
              <span className="text-[10px] text-subtle mt-1">{formattedDate}</span>
            </div>
          </div>
        ...
        <div>
          {items.map((t, idx) => (
            <div
              key={t.id}
              className={`flex justify-between bg-surface items-center p-4 hover:bg-surface-50 transition-colors cursor-pointer ${
                idx !== items.length - 1 ? "border-b border-soft" : ""
              }`}
            >
              <div className="flex items-center gap-4 overflow-hidden">
                <span className="text-sm text-subtle w-16 shrink-0">{t.category}</span>

                <div className="flex flex-col overflow-hidden">
                  <span className="text-sm font-medium text-main truncate">{t.note || t.category}</span>
  ...
  function buildDayGroups(items) {
    ...
    const DayModalRow = ({ t }) => (
    <div className="flex justify-between items-center p-3 border-b border-soft ">
      <div className="flex items-start gap-3">
        <div className="w-12">
          <span className="block text-xs text-subtle">{t.category}</span>
        </div>
        <div className="flex flex-col">
          <span className="text-sm font-medium text-main">{t.note || t.category}</span>
          <span className="text-xs text-subtle">{t.account} {t.toAccount && `→ ${t.toAccount}`}</span>
        </div>
      </div>
      <MoneyText amount={t.amount} type={t.type} className="text-sm" />
    </div>
  ...
  const CalendarCell = ({ cell }) => {
    if (!cell) return <div className="bg-surface min-h-[88px] p-2" />;
    ...
    return (
      <div
        ...
        className={`bg-surface min-h-[88px] p-2 flex flex-col justify-between cursor-pointer relative transition-colors
      ...
      >
        <div className="flex items-start justify-between gap-2">
          ...
          <span className="text-[10px] text-subtle">{tx.length}</span>
        </div>
   ...
   function DateModal({ dateKey, onClose }) {
    ...
    return (
      <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
        <div className="bg-surface w-full max-w-2xl rounded-xl shadow-2xl overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-soft">
            <div>
              <div className="text-sm font-medium text-main">{pretty}</div>
              <div className="text-xs text-subtle">{selectedDayTransactions.length} transactions</div>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => { onClose(); }} className="p-1 hover:bg-surface-100 rounded-full text-gray-600"><X size={18} /></button>
            </div>
          </div>

          <div className="max-h-[60vh] overflow-y-auto">
            {selectedDayTransactions.length === 0 ? (
              <div className="p-6 text-center text-subtle">No transactions</div>
            ) : (
              selectedDayTransactions.map(t => <DayModalRow key={t.id} t={t} />)
            )}
          </div>

          <div className="p-4 border-t border-soft flex justify-end gap-2">
            <button onClick={() => onClose()} className="px-3 py-2 bg-surface-100 rounded-md text-sm">Close</button>
          </div>
        </div>
      </div>
    );
  ...
  const CalendarGrid = () => {
    ...
    return (
      <Card className="h-full flex flex-col" noPadding>
        <div className="grid grid-cols-7 text-center bg-surface-50 border-b">
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
            <div key={d} className="py-2 text-xs font-semibold text-subtle">{d}</div>
          ))}
        </div>

        <div className="grid grid-cols-7 bg-gray-200 gap-px flex-1">
          {Array.from({ length: startOffset }).map((_, i) => (
            <div key={i} className="bg-surface min-h-[80px]" />
          ))}

          {days.map((day) => (
            <div key={day} className="bg-surface min-h-[80px] p-2 hover:bg-blue-50">
...
const monthlySummary = useMemo(() => {
        ...
        <div className="flex flex-col gap-2">
          {categoryList.map((c) => (
            <div key={c.category}>
              ...
              <div className="w-full bg-surface-100 rounded-full h-2 mt-1">
                <div
  ...
  <div className="flex items-center gap-2">
    <button
      className="p-1.5 hover:bg-surface-100 rounded-full text-muted"
    ...
    <select
      className="text-lg font-bold text-main bg-transparent"
      value={currentMonth || ""}
      onChange={(e) => {
    ...
    >
      <ChevronRight size={18} />
        ...
        <div className="flex items-center gap-2">
          {/* <button className="flex items-center gap-2 px-3 py-1.5 bg-surface-100 rounded-full text-xs font-medium text-gray-600">
            <Search size={14} /> Search
          </button> */}
          <div className="flex items-center gap-2">
  {/* Search Box */}
  <div className="relative">
    <Search size={14} className="absolute left-2 top-1/2 -translate-y-1/2 text-subtle" />
    <input
      type="text"
      placeholder="Search"
      value={searchQuery}
      onChange={(e) => setSearchQuery(e.target.value)}
      className="pl-8 pr-3 py-1.5 bg-surface-100 rounded-full text-xs text-stronger outline-none w-40 focus:w-56 transition-all"
    />
  </div>
  ...
  <select
    ...
    className="px-2 py-1.5 text-xs rounded-full bg-surface-100 text-stronger"
  >
  ...
  >
    ...
    {Array.from(new Set(normalizedTransactions.map(t => t.account))).map(acc => (
      <option key={acc} value={acc}>{acc}</option>
          ...
          {/* <button className="p-2 hover:bg-surface-100 rounded-full text-muted"><Star size={16} /></button>
          <button className="p-2 hover:bg-surface-100 rounded-full text-muted"><Filter size={16} /></button> */}
        </div>
      </Card>

      {/* Tabs */}
      <div className="bg-surface-100 p-1 rounded-lg flex text-sm font-medium">
        {["Daily", "Calendar", "Weekly", "Summary"].map((tab) => (
          <button
            key={tab}
            onClick={() => setSubTab(tab)}
            className={`flex-1 py-1.5 rounded-md transition-all ${subTab === tab ? "bg-surface text-stronger shadow-sm" : "text-muted hover:text-gray-700"}`}
          >
            {tab}
          </button>
        ))}
      ...
      <Card className="py-3">
        <div className="grid grid-cols-3 divide-x divide-gray-100">
          <div className="flex flex-col items-center gap-1">
            <span className="text-[10px] font-bold text-subtle uppercase">Income</span>
            <span className="font-semibold text-blue-500">{monthlyIncome.toFixed(2)}</span>
          </div>

          <div className="flex flex-col items-center gap-1">
            <span className="text-[10px] font-bold text-subtle uppercase">Expenses</span>
            <span className="font-semibold text-red-500">{monthlyExpense.toFixed(2)}</span>
          </div>

          <div className="flex flex-col items-center gap-1">
            <span className="text-[10px] font-bold text-subtle uppercase">Total</span>
            <span className="font-semibold text-main">{monthlyTotal.toFixed(2)}</span>
          </div>
        </div>
      </Card>
  ...
  <div className="w-20 h-20 bg-surface-100 rounded-full flex items-center justify-center shadow-sm">
    <svg
      xmlns="http://www.w3.org/2000/svg"
      className="w-10 h-10 text-gray-300"
      fill="none"
              ...
              return (
                <div key={wk.mondayKey}>
                  <Card noPadding className="mb-2">
                    <div
                      className="flex justify-between items-center px-4 py-3 cursor-pointer hover:bg-surface-50 transition-colors"
                    ...
                    >
                      <div>
                        ...
                        <div className="text-xs text-subtle">{wk.items.length} transactions</div>
                      </div>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="text-sm text-main font-semibold">{wk.total.toFixed(2)}</div>
                      </div>
                    </div>
                  </Card>
  ...
  <SummaryView />
            ...
            <Card noPadding>
              ...
              <div className="grid grid-cols-7 bg-gray-200 gap-px">
                {calendarForMonth.weeks.length === 0 ? (
                  <div className="col-span-7 p-6 text-center text-subtle">No data for this month</div>
          ...
          <div className="text-center text-subtle mt-20">Not implemented yet</div>
        )}
      </div>
